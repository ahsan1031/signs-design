var Gantt = function() {
    ...
    var r = {
        ...
        to_string(t, e = !1) {
            ...
            const s = this.get_date_values(t).map(((t, e) => (1 === e && (t += 1), h(t + "", 6 === e ? 3 : 2, "0")))),
                i = `${s[0]}-${s[1]}-${s[2]}`,
                n = `${s[3]}:${s[4]}:${s[5]}.${s[6]}`;
            return i + (e ? " " + n : "")
        },
        format(t, e = "YYYY-MM-DD HH:mm:ss.SSS", s = "en") {
            const i = new Intl.DateTimeFormat(s, {
                ...
                a = this.get_date_values(t).map((t => h(t, 2, 0))),
                o = {
                    YYYY: a[0],
                    MM: h(+a[1] + 1, 2, 0),
                    DD: a[2],
            ...
            return Object.keys(o).sort(((t, e) => e.length - t.length)).forEach((t => {
                r.includes(t) && (r = r.replace(t, `$${d.length}`), d.push(o[t]))
            })), d.forEach(((t, e) => {
                r = r.replace(`$${e}`, t)
            })), r
        },
        ...
        add(r, h, d) {
            h = parseInt(h, 10);
            const p = [r.getFullYear() + (d === t ? h : 0), r.getMonth() + (d === e ? h : 0), r.getDate() + (d === s ? h : 0), r.getHours() + (d === i ? h : 0), r.getMinutes() + (d === n ? h : 0), r.getSeconds() + (d === a ? h : 0), r.getMilliseconds() + (d === o ? h : 0)];
            return new Date(...p)
        },
        start_of(r, h) {
            const d = {
            ...
            const _ = [r.getFullYear(), p(t) ? 0 : r.getMonth(), p(e) ? 1 : r.getDate(), p(s) ? 0 : r.getHours(), p(i) ? 0 : r.getMinutes(), p(n) ? 0 : r.getSeconds(), p(a) ? 0 : r.getMilliseconds()];
            return new Date(..._)
        },
        clone(t) {
    ...
    d.on = (t, e, s, i) => {
        i ? d.delegate(t, e, s, i) : (i = s, d.bind(t, e, i))
    }, d.off = (t, e, s) => {
    ...
    }, d.closest = (t, e) => e ? e.matches(t) ? e : d.closest(t, e.parentNode) : null, d.attr = (t, e, s) => {
        if (!s && "string" == typeof e) return t.getAttribute(e);
        if ("object" != typeof e) t.setAttribute(e, s);
        else
            for (let s in e) d.attr(t, s, e[s])
    };
    class l {
        constructor(t, e) {
        ...
        prepare_values() {
            this.invalid = this.task.invalid, this.height = this.gantt.options.bar_height, this.x = this.compute_x(), this.y = this.compute_y(), this.corner_radius = this.gantt.options.bar_corner_radius, this.duration = r.diff(this.task._end, this.task._start, "hour") / this.gantt.options.step, this.width = this.gantt.options.column_width * this.duration, this.progress_width = this.gantt.options.column_width * this.duration * (this.task.progress / 100) || 0, this.group = p("g", {
                class: "bar-wrapper " + (this.task.custom_class || ""),
                "data-id": this.task.id
            }), this.bar_group = p("g", {
                class: "bar-group",
                append_to: this.group
            }), this.handle_group = p("g", {
                class: "handle-group",
                append_to: this.group
            })
        }
        ...
        draw_bar() {
            ...
            }), _(this.$bar, "width", 0, this.width), this.invalid && this.$bar.classList.add("bar-invalid")
        }
        draw_progress_bar() {
            this.invalid || (this.$bar_progress = p("rect", {
                x: this.x,
                y: this.y,
                ...
                class: "bar-progress",
                append_to: this.bar_group
            }), _(this.$bar_progress, "width", 0, this.progress_width))
        }
        draw_label() {
            p("text", {
        ...
        draw_resize_handles() {
            ...
            const t = this.$bar;
           this.task.progress && this.task.progress < 100 && (this.$handle_progress = p("polygon", {
        ...
        setup_click_event() {
            d.on(this.group, "focus " + this.gantt.options.popup_trigger, (t => {
                this.action_completed || (this.show_popup(), this.gantt.unselect_all(), this.group.classList.add("active"))
            })), d.on(this.group, "dblclick", (t => {
                this.action_completed || this.gantt.trigger_event("click", [this.task])
            }))
        }
        show_popup() {
            if (this.gantt.bar_being_dragged) return;
            const t = r.format(this.task._start, "MMM D", this.gantt.options.language) + " - " + r.format(r.add(this.task._end, -1, "second"), "MMM D", this.gantt.options.language);
        ...
        date_changed() {
            ...
            Number(this.task._start) !== Number(e) && (t = !0, this.task._start = e), Number(this.task._end) !== Number(s) && (t = !0, this.task._end = s), t && this.gantt.trigger_event("date_change", [this.task, e, r.add(s, -1, "second")])
        }
        progress_changed() {
        ...
        compute_start_end_date() {
            const t = this.$bar,
                ...
                s = r.add(this.gantt.gantt_start, e * this.gantt.options.step, "hour"),
                i = t.getWidth() / this.gantt.options.column_width;
            return {
                new_start_date: s,
                new_end_date: r.add(s, i * this.gantt.options.step, "hour")
            }
        }
        compute_progress() {
        ...
        compute_x() {
            const {
                step: t,
                column_width: e
            } = this.gantt.options, s = this.task._start, i = this.gantt.gantt_start;
            let n = r.diff(s, i, "hour") / t * e;
            if (this.gantt.view_is("Month")) {
                n = r.diff(s, i, "day") * e / 30
            }
            return n
        }
        compute_y() {
        ...
        update_progressbar_position() {
            this.invalid || (this.$bar_progress.setAttribute("x", this.$bar.getX()), this.$bar_progress.setAttribute("width", this.$bar.getWidth() * (this.task.progress / 100)))
        }
    ...
    class w {
        ...
        setup_wrapper(t) {
            ...
            e ? (this.$svg = e, this.$svg.classList.add("gantt")) : this.$svg = p("svg", {
                append_to: s,
                class: "gantt"
        ...
        setup_tasks(t) {
            this.tasks = t.map(((t, e) => {
                if (t._start = r.parse(t.start), t._end = r.parse(t.end), r.diff(t._end, t._start, "year") > 10 && (t.end = null), t._index = e, !t.start && !t.end) {
                    const e = r.today();
                    t._start = e, t._end = r.add(e, 2, "day")
                }!t.start && t.end && (t._start = r.add(t._end, -2, "day")), t.start && !t.end && (t._end = r.add(t._start, 2, "day"));
                if (r.get_date_values(t._end).slice(3).every((t => 0 === t)) && (t._end = r.add(t._end, 24, "hour")), t.start && t.end || (t.invalid = !0), "string" == typeof t.dependencies || !t.dependencies) {
                    let e = [];
        ...
        update_view_scale(t) {
            this.options.view_mode = t, t === m.DAY ? (this.options.step = 24, this.options.column_width = 38) : t === m.HALF_DAY ? (this.options.step = 12, this.options.column_width = 38) : t === m.QUARTER_DAY ? (this.options.step = 6, this.options.column_width = 38) : t === m.WEEK ? (this.options.step = 168, this.options.column_width = 140) : t === m.MONTH ? (this.options.step = 720, this.options.column_width = 120) : t === m.YEAR && (this.options.step = 8760, this.options.column_width = 120)
        }
        setup_dates() {
        ...
        setup_gantt_dates() {
            ...
            this.gantt_start = r.start_of(this.gantt_start, "day"), this.gantt_end = r.start_of(this.gantt_end, "day"), this.view_is([m.QUARTER_DAY, m.HALF_DAY]) ? (this.gantt_start = r.add(this.gantt_start, -7, "day"), this.gantt_end = r.add(this.gantt_end, 7, "day")) : this.view_is(m.MONTH) ? (this.gantt_start = r.start_of(this.gantt_start, "year"), this.gantt_end = r.add(this.gantt_end, 1, "year")) : this.view_is(m.YEAR) ? (this.gantt_start = r.add(this.gantt_start, -2, "year"), this.gantt_end = r.add(this.gantt_end, 2, "year")) : (this.gantt_start = r.add(this.gantt_start, -1, "month"), this.gantt_end = r.add(this.gantt_end, 1, "month"))
        }
        setup_date_values() {
            this.dates = [];
            let t = null;
            for (; null === t || t < this.gantt_end;) t = t ? this.view_is(m.YEAR) ? r.add(t, 1, "year") : this.view_is(m.MONTH) ? r.add(t, 1, "month") : r.add(t, this.options.step, "hour") : r.clone(this.gantt_start), this.dates.push(t)
        }
        bind_events() {
        ...
        setup_layers() {
            ...
            for (let e of t) this.layers[e] = p("g", {
                class: e,
                append_to: this.$svg
            })
        }
        ...
        make_grid_rows() {
            ...
            let n = this.options.header_height + this.options.padding / 2;
            for (let a of this.tasks) p("rect", {
                x: 0,
                y: n,
                width: s,
                height: i,
        ...
        make_grid_ticks() {
            ...
            for (let i of this.dates) {
                let n = "tick";
                this.view_is(m.DAY) && 1 === i.getDate() && (n += " thick"), this.view_is(m.WEEK) && i.getDate() >= 1 && i.getDate() < 8 && (n += " thick"), this.view_is(m.MONTH) && i.getMonth() % 3 == 0 && (n += " thick"), p("path", {
                    d: `M ${t} ${e} v ${s}`,
                    class: n,
                    append_to: this.layers.grid
                }), this.view_is(m.MONTH) ? t += r.get_days_in_month(i) * this.options.column_width / 30 : t += this.options.column_width
            }
        }
        make_grid_highlights() {
            if (this.view_is(m.DAY)) {
                p("rect", {
                    x: r.diff(r.today(), this.gantt_start, "hour") / this.options.step * this.options.column_width,
                    y: 0,
        ...
        get_date_info(t, e, s) {
            e || (e = r.add(t, 1, "year"));
            const i = {
                    "Quarter Day_lower": r.format(t, "HH", this.options.language),
                    "Half Day_lower": r.format(t, "HH", this.options.language),
                    Day_lower: t.getDate() !== e.getDate() ? r.format(t, "D", this.options.language) : "",
                    Week_lower: t.getMonth() !== e.getMonth() ? r.format(t, "D MMM", this.options.language) : r.format(t, "D", this.options.language),
                    Month_lower: r.format(t, "MMMM", this.options.language),
                    Year_lower: r.format(t, "YYYY", this.options.language),
                    "Quarter Day_upper": t.getDate() !== e.getDate() ? r.format(t, "D MMM", this.options.language) : "",
                    "Half Day_upper": t.getDate() !== e.getDate() ? t.getMonth() !== e.getMonth() ? r.format(t, "D MMM", this.options.language) : r.format(t, "D", this.options.language) : "",
                    Day_upper: t.getMonth() !== e.getMonth() ? r.format(t, "MMMM", this.options.language) : "",
                    Week_upper: t.getMonth() !== e.getMonth() ? r.format(t, "MMMM", this.options.language) : "",
                    Month_upper: t.getFullYear() !== e.getFullYear() ? r.format(t, "YYYY", this.options.language) : "",
                    Year_upper: t.getFullYear() !== e.getFullYear() ? r.format(t, "YYYY", this.options.language) : ""
                },
                n = {
        ...
        make_arrows() {
            ...
            for (let t of this.tasks) {
                ...
                e = t.dependencies.map((e => {
                    ...
                    if (!s) return;
                    const i = new u(this, this.bars[s._index], this.bars[t._index]);
        ...
        set_scroll_position() {
            ...
            if (!t) return;
            const e = r.diff(this.get_oldest_starting_date(), this.gantt_start, "hour") / this.options.step * this.options.column_width - this.options.column_width;
            t.scrollLeft = e
        }
        bind_grid_click() {
            d.on(this.$svg, this.options.popup_trigger, ".grid-row, .grid-header", (() => {
        ...
        bind_bar_events() {
            let t = !1,
                ...
                a = null,
                o = [];
            this.bar_being_dragged = null, d.on(this.$svg, "mousedown", ".bar-wrapper, .handle", ((r, h) => {
                const p = d.closest(".bar-wrapper", h);
                h.classList.contains("left") ? i = !0 : h.classList.contains("right") ? n = !0 : h.classList.contains("bar-wrapper") && (t = !0), p.classList.add("active"), e = r.offsetX, s = r.offsetY, a = p.getAttribute("data-id");
                // const _ = [a, ...this.get_all_dependent_tasks(a)];
                // o = _.map((t => this.get_bar(t))), this.bar_being_dragged = a, o.forEach((t => {
                //     const e = t.$bar;
                //     e.ox = e.getX(), e.oy = e.getY(), e.owidth = e.getWidth(), e.finaldx = 0
                // }))
            })), d.on(this.$svg, "mousemove", (s => {
                if (!(t || i || n)) return;
                const r = s.offsetX - e;
                s.offsetY, o.forEach((e => {
                    const s = e.$bar;
                    s.finaldx = this.get_snap_position(r), this.hide_popup(), i ? a === e.task.id ? e.update_bar_position({
                        x: s.ox + s.finaldx,
                ...
                (t || i || n) && o.forEach((t => t.group.classList.remove("active"))), t = !1, i = !1, n = !1
            })), d.on(this.$svg, "mouseup", (t => {
                this.bar_being_dragged = null, o.forEach((t => {
        ...
        bind_bar_progress() {
            let t = 0,
                ...
                n = null,
                a = null;
            d.on(this.$svg, "mousedown", ".handle.progress", ((o, r) => {
                s = !0, t = o.offsetX, e = o.offsetY;
                const h = d.closest(".bar-wrapper", r).getAttribute("data-id");
                i = this.get_bar(h), n = i.$bar_progress, a = i.$bar, n.finaldx = 0, n.owidth = n.getWidth(), n.min_dx = -n.getWidth(), n.max_dx = a.getWidth() - n.getWidth()
            })), d.on(this.$svg, "mousemove", (e => {
                if (!s) return;
                let a = e.offsetX - t;
                e.offsetY, a > n.max_dx && (a = n.max_dx), a < n.min_dx && (a = n.min_dx);
                const o = i.$handle_progress;
                d.attr(n, "width", n.owidth + a), d.attr(o, "points", i.get_progress_polygon_points()), n.finaldx = a
            })), d.on(this.$svg, "mouseup", (() => {
        ...
        get_snap_position(t) {
            let e, s, i = t;
            return this.view_is(m.WEEK) ? (e = t % (this.options.column_width / 7), s = i - e + (e < this.options.column_width / 14 ? 0 : this.options.column_width / 7)) : this.view_is(m.MONTH) ? (e = t % (this.options.column_width / 30), s = i - e + (e < this.options.column_width / 60 ? 0 : this.options.column_width / 30)) : (e = t % this.options.column_width, s = i - e + (e < this.options.column_width / 2 ? 0 : this.options.column_width)), s
        }
        unselect_all() {
        ...
        show_popup(t) {
            this.popup || (this.popup = new c(this.popup_wrapper, this.options.custom_popup_html)), this.popup.show(t)
        }
        hide_popup() {
